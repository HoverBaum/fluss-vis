...
type StepIO = {
  ...
  end_end: {
    input: { draft2: string; draft: string }
    output: { draft2: string; draft: string }
  }
  writeDraft_RGNGI: {
    input: { prompt: string }
    output: string
...
type StepFunctions = {
  [K in Exclude<FlussStepId, 'start_start' | 'end_end'>]: (
    args: StepIO[K]['input']
...
export async function runFluss(params: {
  ...
  const flowState: FlowState = {
    ...
    end_end: {
      ...
      arguments: [
        {
          sourceStepId: 'writeDraft_lTKMB',
          argumentName: 'draft2',
        },
        {
          sourceStepId: 'writeDraft_RGNGI',
          argumentName: 'draft',
        },
      ],
    },
    writeDraft_RGNGI: {
      id: 'writeDraft_RGNGI',
      status: 'waiting',
      execute: stepFunctions.writeDraft_RGNGI,
      arguments: [
        {
          sourceStepId: 'start_start',
          argumentName: 'prompt',
        },
      ],
    },
    writeDraft_lTKMB: {
      id: 'writeDraft_lTKMB',
      status: 'waiting',
      execute: stepFunctions.writeDraft_lTKMB,
      arguments: [
        {
          sourceStepId: 'start_start',
          argumentName: 'prompt',
  ...
  return new Promise<StepIO['end_end']['output']>((resolve, reject) => {
    ...
    const processSteps = async () => {
      ...
      if (runnableSteps.length === 0) {
        if (Object.values(flowState).every((step) => step.status === 'done')) {
          ...
          if (!flowState.end_end.result) {
            return reject(new Error('End step has no result'))
          }
          return resolve(flowState.end_end.result)
        } else {
          return reject(
